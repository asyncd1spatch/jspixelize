...
export class RangeNumberPair {
  constructor(public rangeEl: HTMLInputElement, public numberEl: HTMLInputElement) {
...
export class RadioGroup {
  ...
  constructor(containerSelector: string, public name: string) {
  ...
  get value(): string {
    const selected = this.root.querySelector<HTMLInputElement>(`input[name="${this.name}"]:checked`);
...
export class FileInput {
  ...
  constructor(public el: HTMLInputElement) {
    this.el.addEventListener("change", () => void this.emitter.fire(this.el.files));
  }
