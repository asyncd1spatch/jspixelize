...
export class PixelizerGenerator {
  parseCustomPalette(paletteStr: string): Palette {
    ...
    if (hexCodes.some((h) => !/^[0-9a-fA-F]{6}$/.test(h))) {
      throw new Error("Invalid hex code format. Use 6-digit codes like FFFFFF.");
    }
  ...
  sampleLabPixels(imageData: RawImageData, maxSamples: number): LABColor[] {
    ...
    for (let i = 0; i < data.length; i += 4 * samplingRate) {
      if (data[i + 3]! > 0) pixels.push(this.rgbToLab([data[i]!, data[i + 1]!, data[i + 2]!]));
    }
    return pixels;
  }
  ...
  kmeans(data: LABColor[], k: number, maxIterations = 20): LABColor[] {
    ...
    const uniquePoints = Array.from(new Set(data.map((p) => JSON.stringify(p)))).map((s) => JSON.parse(s) as LABColor);
    ...
    for (let iter = 0; iter < maxIterations; iter++) {
      const clusters: LABColor[][] = Array.from({ length: centroids.length }, () => []);
      for (const p of data) {
        let best = 0;
        let min = Infinity;
      ...
      for (let i = 0; i < centroids.length; i++) {
        ...
        if (!cluster.length) {
          ...
          newCentroids[i] = [sum[0] / cluster.length, sum[1] / cluster.length, sum[2] / cluster.length];
        }
        if (this.labColorDistanceSq(newCentroids[i]!, centroid) > 1e-4) converged = false;
      }
  ...
  applyPaletteQuantization(imageData: RawImageData, paletteRGB: Palette): RawImageData {
  ...
  estimateDownscaleResolution(w: number, h: number, relativeScale: number): number {
    const longest = Math.max(w, h);
    return Math.max(1.0, Math.round(longest * (2.0 / Math.sqrt(longest * relativeScale * 0.5))));
  }

  resizeNearestNeighbor(imageData: RawImageData, newWidth: number, newHeight: number): RawImageData {
  ...
  modeDownscale(
    ...
    const selectDominantPixel = (colors: RGBAColor[], counts: number[], used: number): RGBAColor => {
      if (used === 0) return [0, 0, 0, 0];
      let bestIdx = 0, bestScore = -Infinity, bestLum = Infinity;
      for (let i = 0; i < used; i++) {
        const color = colors[i]!;
        const count = counts[i]!;
        const lum = 0.2126 * color[0] + 0.7152 * color[1] + 0.0722 * color[2];
        const score = count * (1.0 + weightC * (1.0 - lum / 255.0));
        if (score > bestScore || (Math.abs(score - bestScore) < 1e-6 && lum < bestLum)) {
          bestScore = score;
          bestIdx = i;
          bestLum = lum;
        }
    ...
    for (let x = 0; x < inW; x++) {
      for (let yOut = 0; yOut < outH; yOut++) {
        ...
        const colors: RGBAColor[] = [], counts: number[] = [];
        let used = 0;
        for (let y = yStart; y < yEnd; y++) {
          const idx = (y * inW + x) * 4;
          const r = inData[idx]!, g = inData[idx + 1]!, b = inData[idx + 2]!, a = inData[idx + 3]!;
          let found = -1;
          for (let i = 0; i < used; i++) {
            if (colors[i]![0] === r && colors[i]![1] === g && colors[i]![2] === b && colors[i]![3] === a) {
              found = i;
              break;
            }
          }
        ...
        verticalData.set(selectDominantPixel(colors, counts, used), (yOut * inW + x) * 4);
      }
    }
    ...
    for (let yOut = 0; yOut < outH; yOut++) {
      for (let xOut = 0; xOut < outW; xOut++) {
        ...
        outData.set(selectDominantPixel(colors, counts, used), (yOut * outW + xOut) * 4);
      }
    }
    return { data: outData, width: outW, height: outH };
  }

  async processImage(mainImageData: RawImageData, options: PixelizerOptions): Promise<PixelizerResults> {
    ...
    if (options.paletteMode === "custom") {
      ...
      if (labPixels.length === 0) throw new Error("No non-transparent pixels found for palette generation.");

      const centroidsLab = this.kmeans(labPixels, options.nColors);
      paletteRGB = centroidsLab.map((lab) => this.labToRgb(lab));
    }

    const quantizedImageData = this.applyPaletteQuantization(mainImageData, paletteRGB);

    let finalImageData = quantizedImageData;
    if (options.shouldPixelize) {
      const scalePx = this.estimateDownscaleResolution(origW, origH, options.relativeScale);
      ...
      finalImageData = this.resizeNearestNeighbor(downscaledImageData, finalW, finalH);
    }
