...
import { Button, FileInput, RadioGroup, RangeNumberPair, Status } from "./scaffold.js";
...
class BrowserImageProcessor {
  ...
  async processImageFile(mainFile: File, options: WebProcessOptions) {
    ...
    if (options.fixPaletteFile) {
      const paletteImage = await this.loadImageElementFromFile(options.fixPaletteFile);
      fixPaletteData = this.extractImageData(paletteImage);
    }

    const libOptions: PixelizerOptions = { ...options, fixPaletteData };
    const result = await this.lib.processImage(mainImageData, libOptions);
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  const fixPaletteInputEl = document.getElementById("fix-palette-input") as HTMLInputElement;
  const customPaletteEl = document.getElementById("custom-palette") as HTMLTextAreaElement;
  const processBtnEl = document.getElementById("process-btn") as HTMLButtonElement;
  ...
  const resultPalette = document.getElementById("result-palette") as HTMLElement;
  const nColorsInput = document.getElementById("n-colors") as HTMLInputElement;
  const nColorsSlider = document.getElementById("n-colors-slider") as HTMLInputElement;
  const relativeScaleInput = document.getElementById("relative-scale") as HTMLInputElement;
  const relativeScaleSlider = document.getElementById("relative-scale-slider") as HTMLInputElement;
  const weightCInput = document.getElementById("weight-c") as HTMLInputElement;
  const weightCSlider = document.getElementById("weight-c-slider") as HTMLInputElement;
  const outputSizeInput = document.getElementById("output-size") as HTMLInputElement;
  const noDownscaleCheckbox = document.getElementById("no-downscale") as HTMLInputElement;
  const enablePixelizationCheckbox = document.getElementById("enable-pixelization") as HTMLInputElement;
  const kmeansOptions = document.getElementById("kmeans-options") as HTMLElement;
  const customOptions = document.getElementById("custom-options") as HTMLElement;
  ...
  if (allElements.some(el => !el)) {
    throw new Error("A required DOM element was not found. Check your HTML IDs.");
  }
  ...
  enablePixelizationCheckbox.addEventListener("change", () => {
    document.getElementById("pixelization-options")
  ...
  async function handleProcessClick(): Promise<void> {
    ...
    try {
      const processingOptions: WebProcessOptions = {
        ...
        outputSize: outputSizeInput.value ? parseInt(outputSizeInput.value, 10) : null,
      ...
      renderResultImageAndPalette(result.dataUrl, result.paletteRGB, inputFileName, processingOptions.shouldPixelize);
  ...
  function renderResultImageAndPalette(
    ...
    palette.forEach((rgb) => {
      ...
      d.title = `#${rgb.map((c) => c.toString(16).padStart(2, "0")).join("").toUpperCase()}`;
